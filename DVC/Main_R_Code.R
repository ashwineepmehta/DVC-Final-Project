...
MainData$Year <- as.numeric(format(MainData$Date,'%Y'))
...
#ordered_data <- crashes_by_year[order(Crash, -Year),]
...
vizpart_colored <- ggplot(data = MainData, mapping = aes(x = Year, fill = max_flag)) + geom_bar() + scale_fill_manual(values=c("grey50","red")) + theme(text = element_text(size = 12, face = "bold"), panel.grid.major = element_blank(), plot.title = element_text(hjust = 0.5), panel.grid.minor = element_blank(), panel.background = element_rect(fill = "transparent",colour = NA), plot.background = element_rect(fill = "transparent",colour = NA)) + ggtitle("Number of crashes per year from 1908 to 2021" ) + xlab("Year") + ylab("Number of Crashes") + geom_text(size = 3, stat='count', aes(label=..count..), vjust=-1) + scale_x_continuous(breaks = seq(from = 1908, to = 2022, by = 10)) + coord_cartesian(clip = "off") + guides(fill = "none")
...
vizpart2 <- ggplot(data = line_data, aes(x = Year, y = Crashes, group = 1)) + geom_line() + theme(text = element_text(size = 12, face = "bold"), panel.grid.major = element_blank(), plot.title = element_text(hjust = 0.5), panel.grid.minor = element_blank(), panel.background = element_rect(fill = "transparent",colour = NA), plot.background = element_rect(fill = "transparent",colour = NA)) + ggtitle("Trend of crashes from 1908 to 2021" ) + xlab("Year") + ylab("Number of Crashes") + scale_x_continuous(breaks = seq(from = 1908, to = 2022, by = 10))

vizpart2

plot_grid(vizpart_colored, vizpart2, ncol = 1, nrow = 2)
...
plot_grid(Viz2b, Viz2c, ncol = 1, nrow = 2)
...
plot_grid(Viz7a, Viz7c, ncol = 1, nrow = 2)
